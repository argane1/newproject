  ...
  return (
    <main className="container mx-auto flex min-h-screen flex-col items-center justify-center gap-6 px-8 text-center">
      ...
      <div className="space-y-2">
        <h1 className="text-4xl font-bold tracking-tight">Welcome back{typeof name === 'string' && name ? `, ${name}` : ""}!</h1>
      ...
      <section className="w-full max-w-lg space-y-4">
        ...
        <form action={addLink} className="space-y-3">
          ...
          <button type="submit" className="w-full rounded-md bg-foreground px-4 py-2.5 text-sm font-semibold text-background transition hover:opacity-90">Add</button>
        </form>
      </section>
          ...
          <ul className="space-y-2">
            {links.map((link) => (
              <li key={link.id} className="flex items-center justify-between rounded-md border border-input px-4 py-3">
                ...
                <button type="button" onClick={() => deleteLink(link.id)} className="rounded-md border px-2 py-1 text-sm transition hover:bg-accent">Delete</button>
              </li>
            ))}
          </ul>
        )}
